...
### Added
- **Extension seam**: opt-in constructs beyond pure markdown.
  A `MarkdownExtension` supplies a `SpanSyntax` (open/close
  delimiter pair + matching flags), content attributes, and an HTML wrapper
  for the clean-copy path; register instances via
  `MarkdownEditorConfiguration.extensions`. Extensions never emit ranges —
  the parser derives all span geometry, so a misbehaving extension can at
  worst restyle its own span. Marker mute/shrink, caret reveal, incremental
  restyle, table cells, and rich copy are handled generically. Registered
  extensions can change at runtime; all parse caches key on the registry.
- `HighlightExtension` (`==text==`) and `StrikethroughExtension`
  (`~~text~~`), the former built-ins repackaged as extensions.
...
- **Breaking**: `==highlight==` and `~~strikethrough~~` are no longer part of
  ...
  previous behavior, register both extensions:
  ...
  span detection (toggle-off, selection state) requires the extension.
